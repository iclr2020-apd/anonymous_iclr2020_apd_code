...
class MODEL(object):
	def __init__(self, config, task_id, test=False):
		...
		for i in [0, 1]:
			...
			if self.c.mask_init == 'random':
				m = self.create_variable('task%d'%self.tid, 'mask/layer%d'%i, self.c.dims[i+1])
			elif self.c.mask_init == 'one':
				m = self.create_variable('task%d'%self.tid, 'mask/layer%d'%i, None, True, initializer=tf.ones(self.c.dims[i+1]))
			else:
				print('ERROR, wrong mask init -> %s'%self.c.mask_init)
				sys.exit()
		for i in [2, 3]:
			# NOTE transpose
			...
			if self.c.mask_init == 'random':
				...
				m = self.create_variable('task%d'%self.tid, 'mask/layer%d'%i, None, True, initializer=tf.ones(self.c.dims[i+2]))
			else:
				print('ERROR, wrong mask init -> %s'%self.c.mask_init)
				sys.exit()
	...
	def create_variable(self, scope, name, shape, trainable=True, add_param=True,
		...
		with tf.variable_scope(scope):
			...
			else: # Initialization with specific parameter
	...
	def save(self, model_dir, option=None, remove_ploc=None):
		#save_vars = [vv for vv in tf.trainable_variables() if not 'id%d_local'%prv_local_id in vv.name]
		#"""
	...
	def load_params(self, tid, model_dir, load_type='all', option=None):
		...
		def various_cond(var, cond):
			...
			#print('[%s] %s, condA:%s, condB:%s -> %s'%(load_type, var.name, condA, condB, output))
			return output
	...
	def create_task_parameters(self, list):
		...
		for key in list.keys():
			...
			try:
				...
				pass
				#print('variable[%s] is passed, thus not be created.'%_key)
		print('DONE')
	...
	def bernoulli_generation(self, z):
		sz = tf.nn.sigmoid(z)
		# σ(lnϵ−ln(1−ϵ)+lnθi(y)−ln(1−θi(y)))
		epsilon = tf.random_uniform(z.shape)
		differentiable_z = tf.nn.sigmoid(tf.log(epsilon + 1e-8) - tf.log(1-epsilon) + tf.log(sz) - tf.log(1-sz))
		return differentiable_z


	def gen_mask(self, mask):
		if self.c.mask_type == 'deter':
			mask = mask #mask = tf.nn.sigmoid(mask)
		elif self.c.mask_type == 'sigmoid':
			mask = tf.nn.sigmoid(mask)
		elif self.c.mask_type == 'ber':
			mask = self.bernoulli_generation(mask)
		else:
			print('ERROR, wrong mask type -> %s'%self.c.mask_type)
			sys.exit()
		return mask
	...
	def build_model_test(self, is_hierarchy=False, is_single_group=True):
		...
		for i in [0, 1]:
			...
			if is_hierarchy and self.tid < self.local_id:
				...
				if is_single_group:
					...
					#print(' [debug] test_layer%d, get aw + local'%i)
				...
				pass
			# NOTE	case 1: self.tid < self.c.clustering_iter
			#		case 2: local_id < self.tid < local_id + self.c.clustering_iter (not clustered yet)
			# 		case 3: self.tid < local_id, but is_single_group
			if is_single_group:
				#print(' [debug] test_layer%d, get aw only'%i)
			...
			# Adaptive parameters
			print(' [*] layer:%d, is_hierarchy: %s, is_single_group: %s'%(i, is_hierarchy, is_single_group))
		...
		for i in [2, 3]:
			...
			# NOTE Save the # of activated parameters
			print(' [*] layer:%d, is_hierarchy: %s, is_single_group: %s'%(i, is_hierarchy, is_single_group))
	...
	def build_model(self, is_hierarchy=False, is_single_group=True):
		...
		#_step = self.c.task_order[self.c.order_type].index(self.tid)
		for i in [0, 1]:
			# Shared weights
				...
				for j in range(self.tid):
					...
					if is_hierarchy and j < self.local_id:
						...
						if not is_single_group:
							#print(' [debug] train_layer%d, get aw + local'%i)
							local_shared = self.get_variable('id%d_local%d'%(self.local_id, group_info), 'aw_group/layer%d'%i, False, reuse=True)
					...
					# 		case 3: self.tid < local_id, but is_single_group
					if is_single_group:
						#print(' [debug] train_layer%d, get aw only'%i)
						paw = self.get_variable('task%d'%j, 'aw_all/layer%d'%i, True, reuse=True)
						full_paw = paw
		...
		for i in [2, 3]:
			...
			if self.tid == 0:#self.c.task_order[self.c.order_type][0]:
				weight_decay += self.c.wd_rate * tf.nn.l2_loss(sw)
			else:
				# NOTE Handle previous aw
				for j in range(self.tid):#self.c.task_order[self.c.order_type][:_step]:
					local_shared_list = []
					pmask = self.get_variable('task%d'%j, 'mask/layer%d'%i, self.remask, reuse=True)
					g_pmask = self.gen_mask(pmask)
	...
	def k_means_clustering(self, prv_cent = None, is_decomposed = False):
		...
		get_cents = []
		#_step = self.c.task_order[self.c.order_type].index(self.tid)
		#_k = int(self.c.k_centroides * (self.c.task_order[self.c.order_type].index(self.tid)+1) / self.c.clustering_iter)
		...
		for i in range(4):
			full_aw = []
			only_aw = []
			for tid in range(self.tid+1):#self.c.task_order[self.c.order_type][:_step+1]:
			...
			self.aw_consolidation2(_k, i, np.array(full_aw), np.array(only_aw), assignment_values)
	...
	def aw_consolidation2(self, _k, layer_id, full_aws, only_aws, cluster_info):
		op_list = []
		for _c in range(_k):
		...
		if not self.test:
			...
			for ind, (grad, var) in enumerate(grads_and_vars):
				...
				#"""
					...
					print('%s is not differentiable.'%var.name)
					#pdb.set_trace()
					pass
				#"""
