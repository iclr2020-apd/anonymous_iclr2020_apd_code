"""
Additive Parameter Decomposition with Hierarchical Knowledge Consolidation
"""
...
parser.add_argument('--results_path', type=str, default='./results/cifar100/apd2/')

parser.add_argument('--dropout_rate', type=float, default=1.0, help='0.5 if dropout, else 1.0')
parser.add_argument('--memory_usage', type=float, default=0.95)

#hyp for apd
parser.add_argument('--mask_hyp', type=float, default=0)
parser.add_argument('--approx_hyp', type=float, default=100.)
parser.add_argument('--l1_hyp', type=float, default=4e-4)

parser.add_argument('--mask_type', type=str, default='sigmoid')
parser.add_argument('--mask_init', type=str, default='random')
parser.add_argument('--mask_retrainable', type=str, default='off', choices=['on', 'off'])

parser.add_argument('--order_type', type=str, default='orderA')
parser.add_argument('--data_type', type=str, default='default', choices=['defailt', 'superclass'])

#parser.add_argument('--max_depth', type=int, default=2)
parser.add_argument('--e_gap_hyp', type=float, default=1e-2)
parser.add_argument('--clustering_iter', type=int, default=5)
parser.add_argument('--k_centroides', type=int, default=2)

parser.add_argument('--forget_task', type=int, default=1)
parser.add_argument('--forget_point', type=int, default=4)


FLAGS = parser.parse_args()
import cifar100_apd2 as cifar100

FLAGS.dims = [3, 20, 50, 3200, 800, 500, 10]
os.environ['CUDA_VISIBLE_DEVICES'] = FLAGS.gpu_num

# NOTE (default) flat=False, one_hot=True
train_data, val_data = data_loader.cifar100_python('/disk/datasets/cifar-100-python/train', group=FLAGS.n_classes, validation=True)
test_data = data_loader.cifar100_python('/disk/datasets/cifar-100-python/test', group=FLAGS.n_classes)
...
if FLAGS.data_type == 'default':
	...
	train_data, val_data = data_loader.cifar100_superclass_python('/disk/datasets/cifar-100-python/train', group=FLAGS.n_classes, validation=True)
	test_data = data_loader.cifar100_superclass_python('/disk/datasets/cifar-100-python/test', group=FLAGS.n_classes)
...
for idx, _t in enumerate(FLAGS.task_order[FLAGS.order_type][:FLAGS.n_tasks]):
	...
	for iidx, _i in enumerate(FLAGS.task_order[FLAGS.order_type][:(idx+1)]):
		...
		# REQUIRED:
		"""
		1. (idx+1) < FLAGS.clustering_iter  ==> is_hier=False
		2. (idx+1) == FLAGS.clustering_iter ==> is_hier=True, model.assign_list = assign_list
		3. (idx+1) > FLAGS.clustering_iter & (iidx+1) =< local_id) ==> is_hier=True, model.assign_list = assign_list
		4. (idx+1) > FLAGS.clustering_iter & (iidx+1) > local_id) ==> is_hier=False
		"""
		# NOTE when after first consolidation, for newly introduced adaptive weights which is not consolidated yet.
		if ((idx+1) >= FLAGS.clustering_iter) and (local_id >= (iidx+1)):
			is_hier = True
			model.assign_list = assign_list
			#print('is_hier: %s ,assign_list[0]:%s'%(is_hier, assign_list[0]))
			#print('TRUE, %d >= %d && %d >= %d'%(idx+1, FLAGS.clustering_iter, FLAGS.clustering_iter * int((idx+1)/FLAGS.clustering_iter), iidx+1))
		else:
			is_hier = False
			#print('is_hier: %s ,assign_list[0]:%s'%(is_hier, assign_list[0]))
			#print('TRUE, %d >= %d && %d >= %d'%(idx+1, FLAGS.clustering_iter, FLAGS.clustering_iter * int((idx+1)/FLAGS.clustering_iter), iidx+1))
